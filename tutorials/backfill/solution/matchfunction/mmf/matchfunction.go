...
func (s *matchFunctionService) Run(req *pb.RunRequest, stream pb.MatchFunction_RunServer) error {
	...
	for _, p := range pools {
		...
		if len(tickets) == 0 {
			log.Printf("No tickets found in the given pool,  %s", p.Name)
		} else {
			log.Printf("%d tickets found in the given pool,  %s", len(tickets), p.Name)
		}
		...
		if len(backfills) == 0 {
			log.Printf("No backfills found in the given pool,  %s", p.Name)
		} else {
			log.Printf("%d backfills found in the given pool,  %s", len(backfills), p.Name)
		}
