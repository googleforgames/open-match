...
func (s *backendService) FetchMatches(req *gopb.FetchMatchesRequest, stream gopb.Backend_FetchMatchesServer) error {
	...
	for _, result := range results {
		...
		default:
			err := stream.Send(&gopb.FetchMatchesResponse{Match: result})
			if err != nil {
...
func doFetchMatchesReceiveMmfResult(ctx context.Context, mmfClients *rpc.ClientCache, req *gopb.FetchMatchesRequest, resultChan chan<- mmfResult) error {
	var grpcClient gopb.MatchFunctionClient
	...
	case gopb.FunctionConfig_GRPC:
		...
		grpcClient = gopb.NewMatchFunctionClient(conn)
	// MatchFunction Hosted as a REST service
	case gopb.FunctionConfig_REST:
	...
	for _, profile := range req.GetProfiles() {
		go func(profile *gopb.MatchProfile) {
...
func doFetchMatchesValidateProposals(ctx context.Context, resultChan <-chan mmfResult, channelSize int) ([]*gopb.Match, error) {
...
func matchesFromHTTPMMF(ctx context.Context, profile *gopb.MatchProfile, client *http.Client, baseURL string) ([]*gopb.Match, error) {
	var m jsonpb.Marshaler
	strReq, err := m.MarshalToString(&gopb.RunRequest{Profile: profile})
	if err != nil {
...
func matchesFromGRPCMMF(ctx context.Context, profile *gopb.MatchProfile, client gopb.MatchFunctionClient) ([]*gopb.Match, error) {
	...
	stream, err := client.Run(ctx, &gopb.RunRequest{Profile: profile})
	if err != nil {
...
// ids passed.
func (s *backendService) AssignTickets(ctx context.Context, req *gopb.AssignTicketsRequest) (*gopb.AssignTicketsResponse, error) {
	...
	return &gopb.AssignTicketsResponse{}, nil
}

func doAssignTickets(ctx context.Context, req *gopb.AssignTicketsRequest, store statestore.Service) error {
