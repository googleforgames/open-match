...
type Evaluator struct {
	Binders      func() error
	EvaluateFunc func(*Params) ([]string, error)
}
...
func (s *evaluatorService) Evaluate(stream pb.Evaluator_EvaluateServer) error {
	...
	stats.Record(ctx, matchesPerEvaluateRequest.M(int64(len(matches))))

	// Run the customized evaluator!
	results, err := s.EvaluateFunc(&Params{
		Logger: logrus.WithFields(logrus.Fields{
			"app":       "openmatch",
	...
	stats.Record(ctx, matchesPerEvaluateResponse.M(int64(len(results))))
