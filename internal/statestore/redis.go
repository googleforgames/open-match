...
func newRedis(cfg config.View) Service {
	...
	redisURL := "redis://"
	maskedURL := redisURL
	if cfg.IsSet(consts.RedisPassword) && cfg.GetString(consts.RedisPassword) != "" {
		redisURL += cfg.GetString(consts.RedisUser) + ":" + cfg.GetString(consts.RedisPassword) + "@"
		maskedURL += cfg.GetString(consts.RedisUser) + ":******@"
	}
	redisURL += cfg.GetString(consts.RedisHostName) + ":" + cfg.GetString(consts.RedisPort)
	maskedURL += cfg.GetString(consts.RedisHostName) + ":" + cfg.GetString(consts.RedisPort)

	redisLogger.WithField("redisURL", maskedURL).Debug("Attempting to connect to Redis")

	pool := &redis.Pool{
		MaxIdle:     cfg.GetInt(consts.RedisConnMaxIdle),
		MaxActive:   cfg.GetInt(consts.RedisConnMaxActive),
		IdleTimeout: cfg.GetDuration(consts.RedisConnIdleTimeout),
		Wait:        true,
		DialContext: func(ctx context.Context) (redis.Conn, error) {
			if ctx.Err() != nil {
				return nil, ctx.Err()
			}
			return redis.DialURL(redisURL, redis.DialConnectTimeout(cfg.GetDuration(consts.RedisConnIdleTimeout)), redis.DialReadTimeout(cfg.GetDuration(consts.RedisConnIdleTimeout)))
		},
	}
	healthCheckPool := &redis.Pool{
		MaxIdle:     1,
		MaxActive:   2,
		IdleTimeout: cfg.GetDuration(consts.RedisConnHealthCheckTimeout),
		Wait:        true,
		DialContext: func(ctx context.Context) (redis.Conn, error) {
			if ctx.Err() != nil {
				return nil, ctx.Err()
			}
			return redis.DialURL(redisURL, redis.DialConnectTimeout(cfg.GetDuration(consts.RedisConnHealthCheckTimeout)), redis.DialReadTimeout(cfg.GetDuration(consts.RedisConnHealthCheckTimeout)))
		},
	}
...
func (rb *redisBackend) CreateTicket(ctx context.Context, ticket *pb.Ticket) error {
	...
	if rb.cfg.IsSet(consts.RedisExpiration) {
		redisTTL := rb.cfg.GetInt(consts.RedisExpiration)
		if redisTTL > 0 {
...
func (rb *redisBackend) IndexTicket(ctx context.Context, ticket *pb.Ticket) error {
	...
	var indices []string
	if rb.cfg.IsSet(consts.TicketIndices) {
		indices = rb.cfg.GetStringSlice(consts.TicketIndices)
	}
...
func (rb *redisBackend) FilterTickets(ctx context.Context, filters []*pb.Filter, pageSize int, callback func([]*pb.Ticket) error) error {
	...
	ttl := rb.cfg.GetDuration(consts.RedisIgnoreListTimeToLive)
	curTime := time.Now()
...
func (rb *redisBackend) newConstantBackoffStrategy() backoff.BackOff {
	backoffStrat := backoff.NewConstantBackOff(rb.cfg.GetDuration(consts.BackoffInitInterval))
...
func (rb *redisBackend) newExponentialBackoffStrategy() backoff.BackOff {
	...
	backoffStrat.InitialInterval = rb.cfg.GetDuration(consts.BackoffInitInterval)
	backoffStrat.RandomizationFactor = rb.cfg.GetFloat64(consts.BackoffRandFactor)
	backoffStrat.Multiplier = rb.cfg.GetFloat64(consts.BackoffMultiplier)
	backoffStrat.MaxInterval = rb.cfg.GetDuration(consts.BackoffMaxInterval)
	backoffStrat.MaxElapsedTime = rb.cfg.GetDuration(consts.BackoffMaxElapsedTime)
	return backoff.BackOff(backoffStrat)
}
