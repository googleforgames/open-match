...
proxy-frontend: build/toolchain/bin/kubectl$(EXE_EXTENSION)
	...
	$(KUBECTL) port-forward --namespace $(OPEN_MATCH_KUBERNETES_NAMESPACE) deployment/om-frontend $(FRONTEND_PORT):51504 $(PORT_FORWARD_ADDRESS_FLAG)
...
proxy-backend: build/toolchain/bin/kubectl$(EXE_EXTENSION)
	...
	$(KUBECTL) port-forward --namespace $(OPEN_MATCH_KUBERNETES_NAMESPACE) deployment/om-backend $(BACKEND_PORT):51505 $(PORT_FORWARD_ADDRESS_FLAG)
...
proxy-mmlogic: build/toolchain/bin/kubectl$(EXE_EXTENSION)
	...
	$(KUBECTL) port-forward --namespace $(OPEN_MATCH_KUBERNETES_NAMESPACE) deployment/om-mmlogic $(MMLOGIC_PORT):51503 $(PORT_FORWARD_ADDRESS_FLAG)
...
proxy-synchronizer: build/toolchain/bin/kubectl$(EXE_EXTENSION)
	...
	$(KUBECTL) port-forward --namespace $(OPEN_MATCH_KUBERNETES_NAMESPACE) deployment/om-synchronizer $(SYNCHRONIZER_PORT):51506 $(PORT_FORWARD_ADDRESS_FLAG)

proxy-grafana: build/toolchain/bin/kubectl$(EXE_EXTENSION)
	@echo "User: admin"
	@echo "Password: openmatch"
	$(KUBECTL) port-forward --namespace $(OPEN_MATCH_KUBERNETES_NAMESPACE) deployment/open-match-grafana $(GRAFANA_PORT):3000 $(PORT_FORWARD_ADDRESS_FLAG)

proxy-prometheus: build/toolchain/bin/kubectl$(EXE_EXTENSION)
	$(KUBECTL) port-forward --namespace $(OPEN_MATCH_KUBERNETES_NAMESPACE) deployment/open-match-prometheus-server $(PROMETHEUS_PORT):9090 $(PORT_FORWARD_ADDRESS_FLAG)
...
proxy-ui: build/toolchain/bin/kubectl$(EXE_EXTENSION)
	...
	$(KUBECTL) port-forward --namespace $(OPEN_MATCH_KUBERNETES_NAMESPACE) deployment/om-swaggerui $(SWAGGERUI_PORT):51500 $(PORT_FORWARD_ADDRESS_FLAG)

proxy-demo: build/toolchain/bin/kubectl$(EXE_EXTENSION)
	@echo "View Demo: http://localhost:$(DEMO_PORT)"
	$(KUBECTL) port-forward --namespace $(OPEN_MATCH_KUBERNETES_NAMESPACE) deployment/om-demo $(DEMO_PORT):51507 $(PORT_FORWARD_ADDRESS_FLAG)
